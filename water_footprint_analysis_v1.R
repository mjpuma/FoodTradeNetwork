...
calculate_water_footprints <- function(years) {
  tryCatch({
    ...
    # Calculate water footprints based on direct proportion approach
    water_footprint_intensities <- water_footprint_long %>%
      ...
      summarise(
        # Keep original water volume in km3/year
        WaterFootprint_km3_per_year = sum(WaterFootprint_km3_per_year, na.rm = TRUE),
        # Total production in kg
        Production_kg = sum(Production_kg, na.rm = TRUE),
        .groups = 'drop'
      ) %>%
      # Calculate an intensity for reference/validation only
      mutate(
    ...
    log_msg(paste("Years covered:", paste(sort(unique(water_footprint_intensities$year)), collapse=", ")))
...
calculate_virtual_water_flows <- function(trade_data, water_footprint_data) {
  ...
  # Calculate virtual water flows using the direct proportion approach
  flows <- trade_data %>%
    # Join with water footprint intensities
    left_join(
    ...
    # Calculate virtual water flow using direct proportion
    mutate(
      # Calculate what fraction of production is being traded
      trade_fraction = value_primary / Production_kg,
      # Apply that fraction to the water volume to get virtual water flow in km3
      virtual_water_m3 = WaterFootprint_km3_per_year * trade_fraction * 1e9,
      # Add a check column in km3
      virtual_water_km3 = virtual_water_m3 / 1e9
    ) %>%
    # Remove rows where the trade fraction is invalid
  ...
  # For main crops comparison with Konar
  if(min(trade_data$year) < 2000) {
    main_crops_summary <- flows %>%
      filter(
      ...
      group_by(WF_Crop) %>%
      summarise(
        green_water_km3 = sum(virtual_water_km3[Water_Type %in% 
                                                  c("Green_Rainfed", "Green_Irrigated")], na.rm=TRUE),
        blue_water_km3 = sum(virtual_water_km3[Water_Type == "Blue_Irrigated"], 
                             na.rm=TRUE)
      )
    
    cat("\n\nMain crops summary (for Konar comparison):")
    print(main_crops_summary)
  }
...
main <- function() {
  ...
  tryCatch({
    ...
    if (analysis_type == "period") {
      ...
      # Generate key statistics with proper period averaging
      key_stats <- bind_rows(
        flows_early %>%
          filter(!is.na(Water_Type)) %>%
          group_by(WF_Crop, Water_Type) %>%
          summarise(
            Total_Virtual_Water_km3 = mean(virtual_water_m3, na.rm = TRUE) / 1e9,
            Mean_Intensity_L_per_kg = mean(WF_L_per_kg, na.rm = TRUE),
            Trade_Volume_Mt = mean(value_primary, na.rm = TRUE) / 1e9,
            year = mean(years_early),
            .groups = 'drop'
          ),
