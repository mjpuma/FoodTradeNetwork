...
# Function to save virtual water flows to Excel
# -----------------------------
# 11. Save Virtual Water Flows
# -----------------------------
save_flows_to_excel <- function(flows, output_dir) {
  # Create output directories (main, data, and docs)
  ...
  # Create and save README.md in both the main directory and docs
  readme_text <- "# Virtual Water Flow Excel Files Documentation

## Directory Structure
This directory contains:
- data/: Excel files containing virtual water flow data
- docs/: Detailed documentation

Each Excel file in the data directory contains virtual water flows organized by commodity.
For detailed documentation, please see docs/README.md."
  
  detailed_readme_text <- "# Virtual Water Flow Excel Files Documentation
...
- Three types of water flows are tracked:
  ...
  writeLines(detailed_readme_text, file.path(output_dir, "docs", "README.md"))
  
  # Convert virtual water to km3 for better readability
  flows <- flows %>%
    mutate(virtual_water_km3 = virtual_water_m3 / 1e9)
  
  # Get unique years and water types
  years <- sort(unique(flows$year))
  water_types <- c("Blue_Irrigated", "Green_Irrigated", "Green_Rainfed")
  
  # Process each year
  for(year_val in years) {
    year_data <- flows %>% filter(year == year_val)
    
    # Process each water type
    for(water_type in water_types) {
      # Filter data for current water type
      water_type_data <- year_data %>% 
        filter(Water_Type == water_type)
      
      if(nrow(water_type_data) == 0) next
      
      # Create filename - now in the data subdirectory
      filename <- file.path(output_dir, "data",
                            paste0(year_val, "_", tolower(water_type), "_flows.xlsx"))
      
      # Create workbook
      wb <- createWorkbook()
      
      # Process each commodity
      for(commodity in sort(unique(water_type_data$WF_Crop))) {
        # Filter data for current commodity
        commodity_data <- water_type_data %>%
          filter(WF_Crop == commodity) %>%
          select(ReporterISO3, PartnerISO3, virtual_water_km3) %>%
          rename(
            Exporter = ReporterISO3,
            Importer = PartnerISO3,
            Virtual_Water_km3 = virtual_water_km3
          ) %>%
          arrange(desc(Virtual_Water_km3))
        
        # Add worksheet
        sheet_name <- substr(commodity, 1, 31)  # Excel has 31 character limit
        addWorksheet(wb, sheet_name)
        writeData(wb, sheet_name, commodity_data)
      }
...
main <- function() {
  ...
  tryCatch({
    ...
    if (analysis_type == "period") {
      ...
      # Save flows to Excel files
      log_message("Saving virtual water flows to Excel files...")
      tryCatch({
        save_flows_to_excel(
          flows_early %>% filter(!is.na(Water_Type)), 
          "outputs/early_period_flows"
        )
        save_flows_to_excel(
          flows_late %>% filter(!is.na(Water_Type)), 
          "outputs/late_period_flows"
        )
      }, error = function(e) {
        log_message(paste("Error saving flows to Excel:", e$message))
      })
